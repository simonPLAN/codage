...
int ** Matrice_Hadamard(int nb_Users, int ** matrice){ /* Génération de la matrice d'Hadamard en fonction du nombre d'utilisateurs */
    ...
    int i, j;
    matrice[0][0] = 1;
    for (taille_temp = 2; taille_temp <= taille_matricielle(nb_Users) ; taille_temp*=2){
    ...
    affichage_Matrice(taille_matricielle(nb_Users), matrice);
    return matrice;
}

int ** generation_sequence(int nb_Users, int ** matrice, int **code, int **sequence){ /* Génère une matrice séquence en fonction des codes */
    int i,j,l;
    for(i=0;i<nb_Users;i++){
        for(j=0;j<3;j++){
...
int main (){
    ...
    generation_sequence(nb_Users, matrice, code, sequence);
